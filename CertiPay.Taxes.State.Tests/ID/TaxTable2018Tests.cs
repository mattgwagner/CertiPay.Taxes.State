...
namespace CertiPay.Taxes.State.Tests.ID
{
    ...
    {     
        ...
        public void ID_2018_Checks_And_Balances(decimal grossWages, PayrollFrequency freq, FilingStatus status, int personalAllowances, decimal expected)
        {
            var table = TaxTables.GetForState(StateOrProvince.ID, year: 2018) as Idaho.TaxTable;
        ...
        public void NegativeValues_ID_2018_Checks_And_Balances(decimal grossWages, PayrollFrequency freq, FilingStatus status, int personalAllowances)
        {
            var table = TaxTables.GetForState(StateOrProvince.ID, year: 2018) as Idaho.TaxTable;            
